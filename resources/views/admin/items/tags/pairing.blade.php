    ...
    <hr>
    <p>If a trait is set, this trait will be granted to all offspring that are created using this item, and will list the second parent's species/subtype.
        <br>It will not be set if the parents species + subtype are identical.
        <br>If a species is set, the offspring will always be that species, but the MYO may have traits of either parent ignoring species restrictions.
        ...
        <tbody id="speciesTableBody">
            <tr class="loot-row hide">
                <td class="loot-row-select">
                    {!! Form::select('illegal_species_ids[]', $specieses, null, ['class' => 'form-control item-select', 'placeholder'
                    => 'Select Species']) !!}
                </td>
            ...
            @if(isset($tag->getData()['illegal_species_ids']) && count($tag->getData()['illegal_species_id']) > 0)
                ...
                <tr class="loot-row">
                    <td class="loot-row-select">
                        {!! Form::select('illegal_species_ids[]', $specieses, $illegal_species_id, ['class' => 'form-control item-select',
