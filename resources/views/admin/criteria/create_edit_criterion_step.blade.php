...
@section('admin-title') Criterion Step @endsection

@section('admin-content')
{!! breadcrumbs(['Admin Panel' => 'admin', 'Criteria' => 'admin/data/criteria', 'Edit Criterion' => 'admin/data/criteria/edit/'.$criterionId, ($step->id ? 'Create' : 'Edit').' Criterion Step' => $step->id ? 'admin/data/criteria/'.$criterionId.'/step/'.$step->id : 'admin/data/criteria/'.$criterionId.'/step']) !!}

<h1>{{ $step->id ? 'Edit' : 'Create' }} Step
    @if($step->id)
...
@if($step->id)
<h2 class="mt-5">Step Options @if($step->type === 'options') <button type="button" class="btn btn-primary float-right option-create">+ Add Option</button> @endif</h2>
     ...
     <div class="card p-3 mb-2 pl-0" data-id="{{ $option->id }}">
        ...
        <i class="fas fa-grip-lines-vertical mr-3" style="font-size: 150%; color: rgba(0,0,0,.3); cursor: grab; margin-left: -3px;"></i>
            <div class="flex-grow-1">
                <h4 class="pb-0 mb-0">
                    @if($option->is_active === 0) <i class="fas fa-eye-slash"></i> @else <i class="fas fa-eye"></i> @endif
                    {{ $option->name }}
                </h4>
