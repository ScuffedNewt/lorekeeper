...
<p>By turning on a default, when you edit the {{ $type }} it will add that criterion group with the pre-determined values that you set. You can have as many default groups as you want, and they can even contain the same criteria as another group-- just with different preset values.</p>
<p><strong>Note:</strong> Toggling on a default will not remove any existing criteria, so if you toggle on a default that contains the same criterion as another default, it will simply add another instance of that criterion to the {{ $type }}.</p>
