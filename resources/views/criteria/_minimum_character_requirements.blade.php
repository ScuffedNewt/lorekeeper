@if ($criterion)
    ...
    <div id="calc-{{ isset($id) ? $id : $criterion->id }}" class="ml-5 steps">
        ...
        @if (isset($isAdmin) && $isAdmin)
            ...
            <div class="text-right mb-3 row align-items-end">
                <div class="col" style="min-width:15em;">
                    {!! Form::select(
                        'character_criterion_currency_id[' . (isset($character) ? $character : $character->slug) . '][' . (isset($id) ? $id : $criterion->id) . ']',
                        $reward_currencies,
                        isset($criterion_currency) ? $criterion_currency : $criterion->currency_id,
                        [
        ...
        @else
            {!! Form::hidden(
                'character_criterion[' . (isset($character) ? $character : $character->slug) . '][' . (isset($id) ? $id : $criterion->id) . '][character_criterion_currency_id]',
                isset($criterion_currency) ? $criterion_currency : $criterion->currency_id,
            ) !!}
        @endif
        ...
        @foreach ($criterion->steps->where('is_active', 1) as $step)
            <div class="form-group">
                ...
                @if ($step->type === 'input')
                    {!! Form::number('character_criterion[' . (isset($character) ? $character : $character->slug) . '][' . (isset($id) ? $id : $criterion->id) . '][' . $step->id . ']', $finalValues[$step->id] ?? null, [
                        'class' => 'form-control',
                        'min' => isset($limitByMinReq) ? $minRequirements[$step->id] ?? null : null,
                    ]) !!}
                @elseif($step->type === 'options')
                    ...
                    @endphp
                    {!! Form::select('character_criterion[' . (isset($character) ? $character : $character->slug) . '][' . (isset($id) ? $id : $criterion->id) . '][' . $step->id . ']', $finalOptions, $finalValues[$step->id] ?? null, [
                        'class' => 'form-control selectize',
                        'placeholder' => 'Select an Option',
                    ]) !!}
                @elseif($step->type === 'boolean')
                    {!! Form::checkbox('character_criterion[' . (isset($character) ? $character : $character->slug) . '][' . (isset($id) ? $id : $criterion->id) . '][' . $step->id . ']', 1, $finalValues[$step->id] ?? 0, [
    ...
    <script>
        $(`#calc-{{ isset($id) ? $id : $criterion->id }}.steps`).on('change input', (e) => {
            ...
            formObj['character_criterion_currency'] = {{ isset($criterion_currency) ? $criterion_currency : $criterion->currency_id }};
            formData = formData.filter((item) => item.name.includes('character_criterion[{{ isset($character) ? $character : $character->slug }}][' + calcId + ']'));
            formData.forEach((item) => formObj[item.name.split('[')[3].replace(']', '')] = item.value);
