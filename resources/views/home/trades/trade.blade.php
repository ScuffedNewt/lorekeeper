...
@elseif($trade->status == 'Rejected' && $trade->reason)
    ...
    <div class="card border-danger mb-3"><div class="card-body">{!! nl2br(htmlentities($trade->reason)) !!}</div></div>
...
<script>
    $(document).ready(function() {
        $('#confirmOfferButton').on('click', function(e) {
            e.preventDefault();
            loadModal("{{ url('trades/'.$trade->id.'/confirm-offer') }}", '{{ (Auth::user()->id == $trade->sender_id ? ($trade->is_sender_confirmed ? 'Unconfirm' : 'Confirm') : ($trade->is_recipient_confirmed ? 'Unconfirm' : 'Confirm')) . ' Offer' }}');
        });
        $('#confirmTradeButton').on('click', function(e) {
            e.preventDefault();
            loadModal("{{ url('trades/'.$trade->id.'/confirm-trade') }}", '{{ (Auth::user()->id == $trade->sender_id ? ($trade->is_sender_trade_confirmed ? 'Unconfirm' : 'Confirm') : ($trade->is_recipient_trade_confirmed ? 'Unconfirm' : 'Confirm')) . ' Trade' }}');
        });
