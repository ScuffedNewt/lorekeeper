<script>
    $(document).ready(function() {
        ...
        $('.image-url').on('change', function() {
            // add hide
            $('.image-url-text').addClass('hide');
            if ($(this).val() == '') {
                // unhide image
                $('#image').removeClass('hide');
                //  remove cropper
                $('#cropSelect').removeClass('hide');
                $cropper.attr('src', null);
                // remove all croppie objects
                c.destroy();
                c = null;
                return;
            }
            ...
            $.get(" {{ url('embed') }}", {
                ...
                if (data.success) {
                    ...
                    // set image-name to data.name
                    $('.image-name').val(data.name);
                    // update cropper
                    updateEmbedCropper(data.url);
                } else {
                    // show error
                    $('.image-url-text').html('<div class="alert alert-danger">' + data.error + '</div>');
                    // remove hide
                    $('.image-url-text').removeClass('hide');
