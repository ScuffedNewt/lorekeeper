<script>
    $(document).ready(function() {
        ...
        var state = { mode:'idle', active:null, drag:null, handle:null, startPt:null, selected:null };
        var viewW = 0, viewH = 0;
        ...
        function setLine(g, x1, y1, x2, y2){
            var $g = $(g);
            $g.find('.measure-line').attr({x1:x1,y1:y1,x2:x2,y2:y2});
            $g.find('[data-role="handle1"]').attr({cx:x1,cy:y1});
            $g.find('[data-role="handle2"]').attr({cx:x2,cy:y2});
            positionLabel(g);
        }

        function dist(x1,y1,x2,y2) { var dx=x2-x1, dy=y2-y1; return Math.hypot(dx,dy); }

        function positionLabel(g) {
            var L = getLine(g);
            var midx = (L.x1 + L.x2)/2, midy = (L.y1 + L.y2)/2;
            $(g).find('[data-role="label"]').attr({x: midx + 6, y: midy - 6});
        }
        ...
        function setViewBox(){
            var iw = $img[0].naturalWidth, ih = $img[0].naturalHeight;
            if (!iw || !ih) {
                return;
            }
