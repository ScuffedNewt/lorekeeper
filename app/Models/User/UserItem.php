...
class UserItem extends Model
{
    ...
    **********************************************************************************************/
    ...
    {
        return ($this->count - $this->trade_count - $this->update_count- $this->submission_count- $this->transfer_count);
    }
     ...
     * @return string
     */
    public function getOthers($tradeCount = 0, $updateCount = 0, $submissionCount = 0, $transferCount = 0)
    {
        return $this->getHeldString($this->trade_count - $tradeCount, $this->update_count - $updateCount, $this->submission_count - $submissionCount, $this->transfer_count - $transferCount);
    }
     ...
     * @return string
     */
    private function getHeldString($tradeCount, $updateCount, $submissionCount, $transferCount)
    {
        if(!$tradeCount && !$updateCount && !$submissionCount && !$transferCount) return null;
        $held = [];
        ...
        if($transferCount) array_push($held, $transferCount.' held in Transfer Queue');
        return ('('.implode(', ',$held).')');
    }
}
