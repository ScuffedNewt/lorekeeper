...
class ConvertRewards extends Command {
    ...
    public function handle() {
        ...
        $this->line("Converting level rewards...\n");
        if (Schema::hasTable('level_rewards')) {
            $levelRewards = DB::table('level_rewards')->get();
            $bar = $this->output->createProgressBar(count($levelRewards));
            $bar->start();
            foreach ($levelRewards as $levelReward) {
                Reward::create([
                    'object_model'    => 'App\Models\Level\Level',
                    'object_id'       => $levelReward->level_id,
                    'rewardable_type' => $levelReward->rewardable_type,
                    'rewardable_id'   => $levelReward->rewardable_id,
                    'quantity'        => $levelReward->quantity,
                ]);
            ...
            $this->info('No level rewards to convert.');
        }
