...
class BatchController extends Controller {
    ...
    public function getCreateEditBatch($id = null) {
        ...
        return view(
            'admin.batches.create_edit_batch',
            [
                'batch'         => $batch,
    ...
    public function postCreateEditBatch(Request $request, BatchService $service, $id = null) {
        ...
        } elseif (!$id && $batch = $service->createBatch($data, Auth::user())) {
    ...
    public function postTriggerBatch(Request $request, BatchService $service, $id) {
        ...
        if ($id && $service->triggerBatch($batch)) {
            ...
            foreach ($targetgroups as $key => $item) {
                flash('The following '.strtolower($key).(count($item) == 1 ? ' has' : 's have').' been activated: '.implode(', ', $item).'.')->success();
            }
        } else {
            foreach ($service->errors()->getMessages()['error'] as $error) {
                flash($error)->error();
