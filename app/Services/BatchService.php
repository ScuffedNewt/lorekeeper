    ...
    */
    ...
    public function triggerBatch($batch) {
        ...
        try {
            ...
            foreach ($batch->targets as $target) {
                if (!$this->activateTarget($target)) {
                    throw new \Exception('Unable to activate '.($target->target->name ? $target->target->name : $target->target->title));
                }
            }
            if (!$this->createLog($batch, $targetgroups, (Auth::check() ? Auth::user()->id : null))) {
                throw new \Exception('Unable to log trigger.');
            }
    ...
    private function activateTarget($target) {
        ...
        if (isset($ring->is_active)) {
            $ring->update(['is_active' => 1]);
        } elseif (isset($ring->is_released)) {
            $ring->update(['is_released' => 1]);
        } elseif (isset($ring->is_visible)) {
            $ring->update(['is_visible' => 1]);
            if ($target->target_type == 'News') {
                (new NewsService)->alertUsers();
            } elseif ($target->target_type == 'Sale') {
                (new SaleService)->alertUsers();
            }
        } else {
            return false;
        }
    ...
    private function getAssetModelString($type, $namespaced = true) {
        switch ($type) {
            ...
            case 'concepts':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Concept';
                } else {
                    return 'Concept';
                }
                break;
            ...
            case 'events':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Event';
                } else {
                    return 'Event';
                }
                break;
            ...
            case 'factions':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Faction';
                } else {
                    return 'Faction';
                }
                break;
            ...
            case 'faunas':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Fauna';
                } else {
                    return 'Fauna';
                }
                break;
            ...
            case 'floras':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Flora';
                } else {
                    return 'Flora';
                }
                break;
            ...
            case 'figures':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Figure';
                } else {
                    return 'Figure';
                }
                break;
            ...
            case 'locations':
                if ($namespaced) {
                    return '\App\Models\WorldExpansion\Location';
                } else {
                    return 'Location';
                }
                break;
