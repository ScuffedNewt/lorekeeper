    ...
    */
    ...
    public function approveSubmission($data, $user) {
        ...
        try {
                    ...
                    foreach ($c as $key => $id) {
                        switch ($data['character_rewardable_type'][$ckey][$key]) {
                            ...
                            case 'Points': $pointIds[] = $id;
                                break;
                            case 'Experience': $experienceIds[] = $id;
                                break;
                        }
                    }
            ...
            $experiences = Experience::whereIn('id', $experienceIds)->get()->keyBy('id');
            ...
            if (isset($data['character_currency_id'][$data['character_id']]) && isset($data['character_quantity'][$data['character_id']])) {
                ...
                foreach ($data['character_rewardable_id'][$data['character_id']] as $key => $reward) {
                    ...
                    switch ($data['character_rewardable_type'][$data['character_id']][$key]) {
                        ...
                        case 'Class': if ($data['character_rewardable_quantity'][$data['character_id']][$key]) {
                            addAsset($assets, $data['class'][$reward], $data['character_rewardable_quantity'][$data['character_id']][$key]);
                        } break;
                    }
                }
            }
                ...
                foreach ($data['rewardable_type'] as $key => $type) {
                    ...
                    switch ($type) {
                        ...
                        case 'Experience':
                            ...
                            $reward = Experience::find($data['rewardable_id'][$key]);
                            break;
                    }
                    if (!$reward) {
                        continue;
    ...
    private function createCharacterAttachments($submission, $data, $defaultRewards = null) {
        ...
        $classes = CharacterClass::whereIn('id', $classIds)->get()->keyBy('id');
        $points = Stat::whereIn('id', $pointIds)->get()->keyBy('id');
        $experience = Experience::whereIn('id', $experienceIds)->get()->keyBy('id');
