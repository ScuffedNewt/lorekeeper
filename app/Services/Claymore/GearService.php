...
class GearService extends Service {
    ...
    **********************************************************************************************/
    ...
    public function updateGearCategory($category, $data, $user) {
        ...
        try {
            ...
            if (GearCategory::where('name', $data['name'])->where('id', '!=', $category->id)->exists()) {
    ...
    public function deleteGearCategory($category) {
        ...
        try {
            ...
            if (Gear::where('gear_category_id', $category->id)->exists()) {
                throw new \Exception('A gear with this category exists. Please change its category first.');
            }
    ...
    **********************************************************************************************/
     ...
     * @param \App\Models\User\User $user
     *
     * @return \App\Models\Gear\Gear|bool
     */
    public function createGear($data, $user) {
        ...
        try {
            ...
            if ((isset($data['gear_category_id']) && $data['gear_category_id']) && !GearCategory::where('id', $data['gear_category_id'])->exists()) {
                throw new \Exception('The selected gear category is invalid.');
            }
    ...
    public function updateGear($gear, $data, $user) {
        ...
        try {
            ...
            // More specific validation
            if (Gear::where('name', $data['name'])->where('id', '!=', $gear->id)->exists()) {
                throw new \Exception('The name has already been taken.');
            }
            if ((isset($data['gear_category_id']) && $data['gear_category_id']) && !GearCategory::where('id', $data['gear_category_id'])->exists()) {
                throw new \Exception('The selected gear category is invalid.');
    ...
    public function deleteGear($gear) {
        ...
        try {
            ...
            if (DB::table('user_gears')->where('gear_id', '=', $gear->id)->where('deleted_at', null)->exists()) {
                throw new \Exception('At least one user currently owns this gear. Please remove the gear(s) before deleting it.');
            }
            if (DB::table('loots')->where('rewardable_type', 'Gear')->where('rewardable_id', $gear->id)->exists()) {
                throw new \Exception('A loot table currently distributes this gear as a potential reward. Please remove the gear before deleting it.');
            }
            if (DB::table('prompt_rewards')->where('rewardable_type', 'Gear')->where('rewardable_id', $gear->id)->exists()) {
                throw new \Exception('A prompt currently distributes this gear as a reward. Please remove the gear before deleting it.');
            }
